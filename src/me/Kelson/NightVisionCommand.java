    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        ...
        if(!(sender instanceof Player)) {
            if (cmd.getName().equalsIgnoreCase("nightvision")) {
                if(args.length == 0){
                    sender.sendMessage(Commands.main + ChatColor.RED + "Error: Command usage /nightvision <on/off> [player]");
                }
                if(args.length == 1){
                    sender.sendMessage(Commands.main + ChatColor.RED + "Error: Console cannot use this on itself!");
                }
